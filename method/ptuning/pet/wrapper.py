...
class TransformerModelWrapper:
    ...
    def train(self,
        ...
        if max_steps > 0:
            t_total = max_steps
            # accumulate epoch automatically
        ...
        # freeze
        if freeze_plm:
            for p in cur_model.model.parameters():
                p.requires_grad = False

        # Prepare optimizer and schedule (linear warmup and decay)
        no_decay = ['bias', 'LayerNorm.weight']
        # pretrained model's parameter
        ...
        total_num += sum(p.numel() for p in cur_model.model.parameters())
        trainable_num += sum(p.numel() for p in cur_model.model.parameters() if p.requires_grad)
        ...
        if self.config.prompt_encoder_type == "lstm":
            ...
            total_num += sum(p.numel() for p in cur_model.lstm_head.parameters())
            trainable_num += sum(p.numel() for p in cur_model.lstm_head.parameters() if p.requires_grad)
            total_num += sum(p.numel() for p in cur_model.mlp_head.parameters())
            trainable_num += sum(p.numel() for p in cur_model.mlp_head.parameters() if p.requires_grad)
            total_num += sum(p.numel() for p in cur_model.prompt_embeddings.parameters())
            trainable_num += sum(p.numel() for p in cur_model.prompt_embeddings.parameters() if p.requires_grad)
        elif self.config.prompt_encoder_type == "mlp":
            embedding_parameters = [
                {'params': [p for p in cur_model.mlp.parameters()]},
                {'params': [p for p in cur_model.prompt_embeddings.parameters()]}
            ]
            total_num += sum(p.numel() for p in cur_model.mlp.parameters())
            trainable_num += sum(p.numel() for p in cur_model.mlp.parameters() if p.requires_grad)
            total_num += sum(p.numel() for p in cur_model.prompt_embeddings.parameters())
            trainable_num += sum(p.numel() for p in cur_model.prompt_embeddings.parameters() if p.requires_grad)
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if (step + 1) % gradient_accumulation_steps == 0:
                    ...
                    loss_list = np.append(loss_list, tr_loss - prev_loss)
                    np.save(os.path.join(pattern_iter_output_dir, "loss.npy"), loss_list)
                    prev_loss = tr_loss
                    ...
                    if global_step % self.config.eval_every_step == 0:
                        dev32_scores = self.eval_dev(dev32_data, eval_config, n_gpu)
                        acc_list = np.append(acc_list, dev32_scores["acc"])
                        np.save(os.path.join(pattern_iter_output_dir, "acc.npy"), acc_list)
                        ...
                        elif self.config.task_name in ["rte", "wic", "boolq", "wsc", "copa", "clonedet"]:
                ...
                if 0 < max_steps < global_step or early_stop != -1 and early_stop_epoch >= early_stop:
                    epoch_iterator.close()
                    print("Stop at {}/{}".format(global_step, max_steps))
                    break
    ...
    def eval(self,
             ...
             n_gpu: int = 1,
             show_limit: int = 0) -> Dict:

        eval_dataset = self._generate_dataset(eval_data, show_limit=show_limit)
    ...
    def _convert_examples_to_features(self, examples: List[InputExample], labelled: bool = True, show_limit: int = 0) -> List[InputFeatures]:
        features = []
